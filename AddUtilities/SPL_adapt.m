...
%for trial = 1:num_trials
    ...
    psnr_spl_adapt (trial) = PSNR(current_image, rec_SPL_adapt);
    SPL_adapt_Name = strcat(filename,'_Rate_',num2str(rate_spl_adapt(trial)),'_QP_',num2str(quantizer_bitdepth),...
        '_subrate_',num2str(subrate),'_PSNR_',num2str(psnr_spl_adapt (trial)),'dB.tif');
    imwrite(uint8(rec_SPL_adapt),strcat('./result/SPL_adapt/', SPL_adapt_Name));
    
    disp(['Rate = ' num2str(rate_spl_adapt(trial), '%0.4f') ...
        ' (bpp), PSNR = ' num2str(psnr_spl_adapt(trial), '%0.2f') ' (dB)']);
%end

save([filename '_' algorithm '_SPL_adapt_results.mat'], ...
        'rate_spl_adapt', 'psnr_spl_adapt');
    
    save([filename 'spc_adapt_index.mat'], ...
        'avg_coff');
end
