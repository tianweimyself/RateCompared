...
function [y_q rate, coef] = DPCM_Coding(y, quantizer_bitdepth, num_rows,num_cols,trial)
...
if iscell(y) % quantization for cell form of data (MS-BCS-SPL)
  ...
  [yq, coef] = DPCM_Encode(y,q);
  
  total_pixels = num_rows*num_cols;
  rate = Measurement_Entropy(yq(:),total_pixels);%观测端拿到信息熵
  % yq can be coded using entropy coder, i.e. arithmetic coder. 
  
  y_q = DPCM_Decode(yq,q); %重构端反量化 + prev
  
end


function [i, coef]= DPCM_Encode(y,q)
